 ...
 * @author UEE
 */
public class ResultadosGeneralesPorContexto extends javax.swing.JInternalFrame {
    ...
    /**
     * Creates new form ResultadosGeneralesPorContexto
     */
    public ResultadosGeneralesPorContexto(List<ModeloExamen> modelosExamenes, JDesktopPane jDesktopPane1) {
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Resultados generales contra variables de contexto");
    ...
    private void boton_aceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_boton_aceptarActionPerformed
        ...
        for(int i=0; i<this.modelosExamenes.size(); i++) {
            ...
            this.modelosExamenes.get(i).setPorContexto_variables_seleccionadas(variables_seleccionadas);
            this.modelosExamenes.get(i).setPorContexto_opciones_respuesta(opciones_respuesta);
            this.modelosExamenes.get(i).setPorContexto_promedio_distractores(promedio_distractores);
            this.modelosExamenes.get(i).setPorContexto_porcentaje_distractores(porcentaje_distractores);
    ...
    private void crearGraficas(int indice_modelo_examen) {
        ...
        for(int i=0; i<this.modelosExamenes.get(indice_modelo_examen).getPorContexto_variables_seleccionadas().length; i++) {
            DefaultCategoryDataset dataset = new DefaultCategoryDataset();
            
            for(int j=0; j<this.modelosExamenes.get(indice_modelo_examen).getPorContexto_opciones_respuesta()[i].size(); j++) {
                double valor = Double.valueOf(df.format(this.modelosExamenes.get(indice_modelo_examen).getPorContexto_porcentaje_distractores()[i].get(j)));
                dataset.setValue(valor, "Porcentaje", this.modelosExamenes.get(indice_modelo_examen).getPorContexto_opciones_respuesta()[i].get(j));           
            }
            
            JFreeChart chart = ChartFactory.createBarChart(this.modelosExamenes.get(indice_modelo_examen).getPorContexto_variables_seleccionadas()[i], "Opciones de respuesta", "Porcentaje", dataset, PlotOrientation.VERTICAL, false, true, true);
...
*/
        ...
        this.modelosExamenes.get(indice_modelo_examen).setPorContexto_graficas(porContexto_graficas);
    }
    
    private void guardarImagenes() {
        File dir = new File("temp\\porContexto");                
        ...
        if (dir.mkdirs()) {
            for(int i=0; i<this.modelosExamenes.size(); i++) {
                ...
                */
                File dirModelo = new File("temp\\porContexto\\" + this.modelosExamenes.get(i).getNombreModelo());                                                               
                
                if (dirModelo.mkdirs()) {
                    
                    for(int j=0; j<this.modelosExamenes.get(i).getPorContexto_graficas().size(); j++) {                                                 
                        
                        try {
                            ChartUtilities.saveChartAsPNG(
                                    new java.io.File("temp\\porContexto\\" + this.modelosExamenes.get(i).getNombreModelo() + "\\" + (this.modelosExamenes.get(i).getPorContexto_graficas().get(j).getTitle().getText()) + ".PNG"),
                                    this.modelosExamenes.get(i).getPorContexto_graficas().get(j),
                                    500,
                                    300);                                                        
    ...
    private void pintar_resultados() {
        ...
        for(int k=0; k<this.modelosExamenes.size(); k++) {            
            ...
            for(int i=0; i<this.modelosExamenes.get(k).getPorContexto_variables_seleccionadas().length; i++) {
                out = out + "<tr>";

                out = out + "<td style=\"text-align:center;\"";                
                out = out + " rowspan=\"" + this.modelosExamenes.get(k).getPorContexto_opciones_respuesta()[i].size() + "\">" + this.modelosExamenes.get(k).getPorContexto_variables_seleccionadas()[i] + "</td>";
                
                for(int j=0; j<this.modelosExamenes.get(k).getPorContexto_opciones_respuesta()[i].size(); j++) {
                    /*double por;
                    ...
                    */
                    ...
                    out = out + "<td>" + this.modelosExamenes.get(k).getPorContexto_opciones_respuesta()[i].get(j) + " </td>";
                    out = out + "<td style=\"text-align:center;\">" + df.format(this.modelosExamenes.get(k).getPorContexto_promedio_distractores()[i].get(j).doubleValue()) + "</td>";
                    out = out + "<td style=\"text-align:center;\">" + df.format(this.modelosExamenes.get(k).getPorContexto_porcentaje_distractores()[i].get(j).doubleValue()) + "</td>";
                    out = out + "</tr>";                                        
                }
        ...
        for(int k=0; k<this.modelosExamenes.size(); k++) {            
            ...
            for(int i=0; i<this.modelosExamenes.get(k).getPorContexto_graficas().size(); i++) {                                
                out = out + "<tr>"
                ...
                String nombreArchivo = "\"file:temp/porContexto/" + this.modelosExamenes.get(k).getNombreModelo() + "/" + (this.modelosExamenes.get(k).getPorContexto_graficas().get(i).getTitle().getText()) + ".PNG\"";                                
